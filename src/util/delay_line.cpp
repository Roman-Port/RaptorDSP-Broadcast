...
template<typename T>
delay_line<T>::delay_line(delay_line const& src) {
    //Initialize defaults
    buffer = src.buffer;
    pos = src.pos;
    len = src.len;
...
template<typename T>
void delay_line<T>::configure(int newDelay, T defaultValue) {
    //Sanity check
    assert(newDelay >= 0);
...
template<typename T>
void delay_line<T>::process(const T* input, T* output, int count) {
    //Sanity check
    assert(count >= 0);
...
template class delay_line<float>;
template class delay_line<lv_32fc_t>;